...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            .card, .shadow-sm, .custom-table, .custom-table thead th {
            ...
            /* Make station name larger and centered */
            .station-name {
            ...
            /* Hide buttons, icons and actions column */
            .btn, button, .bi, .app-content button, .app-content .btn {
            ...
            /* Hide footer action cell */
            .custom-table tfoot td:last-child {
                display: none;
            }
            
            /* Adjust columns after hiding action column */
...
<body class="layout-fixed sidebar-expand-lg sidebar-mini sidebar-collapse bg-body-tertiary">
            ...
            <div class="container-fluid">
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    <div class="table-responsive animate-fade-in">
                        ...
                        // Calculate total days in range - ADD THIS LINE
                        ...
                        // Calculate total expected feedbacks for the period (target × ALL days in range, not just active days)
                        $total_expected_feedbacks = $total_days_in_range * $daily_target;
    ...
    <div class="card-body text-center">
        ...
        <!-- Station name header -->
        <h4 class="mb-3 station-name"><?= htmlspecialchars($station_name) ?> - Feedback Report</h4>

        <div class="row">
            <div class="col-md-2">
                <h5 class="mb-1"><?= $total_expected_feedbacks ?></h5>
                <small>Expected Feedbacks</small>
            </div>
            <div class="col-md-2">
                <h5 class="mb-1"><?= $total_feedbacks ?></h5>
                <small>Actual Feedbacks</small>
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-2">
                <h5 class="mb-1"><?= number_format($adjusted_psi_percentage, 1) ?>%</h5>
                <small>PSI</small>
            </div>
        </div>
        ...
        <div class="row mt-3">
            <div class="col-md-12">
                ...
                <?php endif; ?>
                        ...
                        <table  class="table-padding table table-bordered table-hover shadow-sm custom-table">
                            ...
                            <tbody>
                                ...
                                foreach ($all_dates as $date => $data) {
                                    ...
                                    if ($has_data) {
                                        ...
                                        // Achievement status - simplified
                                        if ($target_achieved) {
                                            echo "<td><i class='bi bi-check-circle-fill text-success'></i> <small><strong>Achieved</strong></small></td>";
                                        } else {
                                            echo "<td><i class='bi bi-x-circle-fill text-danger'></i> <small><strong>Not Achieved</strong></small></td>";
                                        }
                                        ...
                                        echo "<td><a href='feedback-details.php?date={$date}&station_id={$station_id}' class='btn btn-sm btn-outline-primary' target='_blank'>View Details</a></td>";
                                    } else {
                                        // No data available for this date
                                        echo "<td><span class='badge bg-secondary'>0</span></td>";
                                        echo "<td><span class='badge bg-info'>{$daily_target}</span></td>";
                                        echo "<td><i class='bi bi-exclamation-triangle-fill text-warning'></i> <small><strong>No Feedback</strong></small></td>";
                                        echo "<td><span class='badge bg-secondary'>0%</span></td>";
    ...
    <tr style="background: linear-gradient(135deg, #28a745, #20c997); color: white; font-weight: bold;">
        ...
        <td class="text-center">
            <span class="badge bg-light text-dark" style="font-weight: bold; font-size: 0.9rem;"><?= $total_feedbacks ?></span>
        </td>
        <td class="text-center">
            <span class="badge bg-light text-dark" style="font-weight: bold; font-size: 0.9rem;"><?= $total_expected_feedbacks ?></span>
        </td>
        <td class="text-center">
            <?php 
            $overall_achievement_percentage = $total_expected_feedbacks > 0 ? ($total_feedbacks / $total_expected_feedbacks) * 100 : 0;
            $overall_achievement_badge = $overall_achievement_percentage >= 100 ? "bg-success" : ($overall_achievement_percentage >= 75 ? "bg-warning" : "bg-danger");
            ?>
            <span class="badge <?= $overall_achievement_badge ?>" style="font-size: 0.9rem;">
                <?= number_format($overall_achievement_percentage, 1) ?>%
            </span>
        </td>
        <td class="text-center">
            <?php 
            $overall_psi_badge = $adjusted_psi_percentage >= 80 ? "bg-success" : ($adjusted_psi_percentage >= 60 ? "bg-warning" : "bg-danger");
            ?>
            <span class="badge <?= $overall_psi_badge ?>" style="font-size: 0.9rem;">
                <?= number_format($adjusted_psi_percentage, 1) ?>%
            </span>
        </td>
        <td class="text-center">
            <form method="POST" action="feedback-report.php" style="display: inline;" target="_blank">
                <input type="hidden" name="from" value="<?= htmlspecialchars($from_date) ?>">
                <input type="hidden" name="to" value="<?= htmlspecialchars($to_date) ?>">
                <button type="submit" class="btn btn-sm btn-light text-dark" style="font-weight: bold;">
                    ...
                    echo '<i class="bi bi-search"></i>';  // Bootstrap calendar icon
                    echo '<p>Search For Feedback Report</p>';
                    echo '</div>';
                }
                ?>
            </div>
